...
function Op01_1_1_antecipacao_de_recebiveis($uniqueid, $origem, $cnpjcpf,$protocoloEASY,$etapa = '1'){
    ...
    if(preg_match("/^(\d{5})$/",$senha)){
        ...
        if($validar_senha){
            ...
            Op01_1_1_antecipacao_de_recebiveis_2($uniqueid, $origem, $cnpjcpf,$protocoloEASY);
        ...
        if(retentar_dado_invalido("Op01_1_1_antecipacao_de_recebiveis","uraHD/5","Senha não digita ou digitado errado")){
            //32
            $indice++;
            tracking($canal, $ddr, $ticket, $indice, 'ANTECIPACAO DE RECEBIVEIS', 'RETORNO', 'SENHA INVALIDA');
...
function Op01_1_1_antecipacao_de_recebiveis_2($uniqueid, $origem, $cnpjcpf,$protocoloEASY){
    global $canal, $ddr, $ticket, $indice, $horaAtual;
    ...
    if($digitado=='1'){
        ...
        verbose("CLIENTE CONFIRMOU A ANTECIPAÇÃO");
        ...
        if($GLOBALS["testeativo"] == 'Y'){
            ...
            verbose("CONFIRMAÇÃO DO CLIENTE : ".$confirm_simula->{'seqAntecipacaoSimulado'});
            if(!tracking_canal_ativo($canal, $ddr, $ticket, $indice)) exit();
        ...
        verbose("CLIENTE CANCELOU A ANTECIPAÇÃO");
        inicializa_ambiente_novo_menu();
        Op01_1_1_antecipacao_de_recebiveis_cancelar($uniqueid, $origem, $cnpjcpf,$protocoloEASY);
    }else{
...
function Op01_1_1_antecipacao_de_recebiveis_cancelar($uniqueid, $origem, $cnpjcpf,$protocoloEASY){
    global $canal, $ddr, $ticket, $indice, $horaAtual;
    //39
    $indice++;
