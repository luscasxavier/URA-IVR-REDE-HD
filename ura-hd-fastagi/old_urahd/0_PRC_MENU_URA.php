...
function M1_Ja_e_credenciado($uniqueid, $origem){
    global $canal, $ddr, $ticket, $indice, $horaAtual;
    if(!tracking_canal_ativo($canal, $ddr, $ticket, $indice)) exit();
    verbose(">>>>> ENTROU NA OPÇÃO 1 JÁ É CREDENCIADO",3);
    //07
    $indice++;
    ...
    if(preg_match("/^(\d{14})$|^(\d{11})$/",$cnpjcpf)){
        ...
        verbose("RETORNO DA VALIDAÇÃO : ".$cnpjcpfValidado->{'possuiContrato'});
...
function M1_1_Principal($uniqueid, $origem, $cnpjcpf,$protocoloEASY, $cnpjcpfValidado){
    global $canal, $ddr, $ticket, $indice, $horaAtual;
    if(!tracking_canal_ativo($canal, $ddr, $ticket, $indice)) exit();
    ...
    //13
    $indice++;
    tracking($canal, $ddr, $ticket, $indice, 'MENU PRINCIPAL - JA E CREDENCIADO', 'PERCURSO', '1 - ANTECIPACAO DE RECEBIVEIS OU 2 - ALTERAR SENHA DE EC OU 3 - AUTORIZAÇÃO BENEFICIO OU 4 - AUTORIZACAO FROTA OU 9 - FALAR COM ATENDENTE');
    ...
    switch ($menu){
        case 1:
            inicializa_ambiente_novo_menu();
            Op01_1_1_antecipacao_de_recebiveis($uniqueid ,$origem ,$cnpjcpf ,$protocoloEASY);
        break;

        case 2:
            inicializa_ambiente_novo_menu();
            Op01_1_2_alterar_senha_de_ec($uniqueid ,$origem ,$cnpjcpf ,$protocoloEASY, $cnpjcpfValidado);
        break;

        case 3:
            inicializa_ambiente_novo_menu();
            M1_1_3_autorizacao_beneficio($uniqueid ,$origem ,$cnpjcpf ,$protocoloEASY, $cnpjcpfValidado);
        break;
        ...
        case 5:
            inicializa_ambiente_novo_menu();
            conta_agili($uniqueid, $origem, $cnpjcpf, $protocoloEASY, $cnpjcpfValidado);
        break;

        case 6:
            inicializa_ambiente_novo_menu();
            suporte_tecnico($uniqueid, $origem, $cnpjcpf, $protocoloEASY, $cnpjcpfValidado);
        break;
        ...
        case 9:
            ...
            if($validarhorario){
                ...
                if($GLOBALS["testeativo"] == 'Y'){
                    ...
                    //dial_fast("gw02-kontac33/213");
                    dial_return("gw02-kontac33/213");
        ...
        default:
            if(retentar_dado_invalido("M1_1_Principal","uraHD/5","Digitou opcao invalida"))M1_1_Principal($uniqueid, $origem, $cnpjcpf,$protocoloEASY, $cnpjcpfValidado);
            else{
...
function conta_agili($uniqueid, $origem, $cnpjcpf, $protocoloEASY, $cnpjcpfValidado){
    global $canal, $ddr, $ticket, $indice, $horaAtual;
    if($GLOBALS["testeativo"] == 'Y'){
        verbose("Entrou no teste");
        $validarhorario = true;
    ...
    if($validarhorario){
        ...
        if($GLOBALS["testeativo"] == 'Y'){
            ...
            //dial_fast("gw02-kontac33/1118");
            dial_return("gw02-kontac33/1118");
...
function suporte_tecnico($uniqueid, $origem, $cnpjcpf, $protocoloEASY, $cnpjcpfValidado){
    global $canal, $ddr, $ticket, $indice, $horaAtual; 
    if($GLOBALS["testeativo"] == 'Y'){
        verbose("Entrou no teste");
        $validarhorario = true;
    ...
    if($validarhorario){
        ...
        if($GLOBALS["testeativo"] == 'Y'){
            ...
            //dial_fast("gw02-kontac33/203");
            dial_return("gw02-kontac33/203");
...
function M2_Deseja_se_credenciar_p2($uniqueid, $origem, $cnpjcpf, $telefone){
    ...
    if($validarhorario){
        ...
        //dial_fast("gw02-kontac33/204");
        dial_return("gw02-kontac33/204");
...
function pesquisa_satisfacao($uniqueid, $origem){
    ...
    //09
    $indice++;
    tracking($canal, $ddr, $ticket, $indice, 'PESQUISA DE SATISFACAO', 'PERCURSO', 'LIGAÇÃO CONTINUADA');

    //10
    $indice++;
    tracking($canal, $ddr, $ticket, $indice, 'PESQUISA DE SATISFACAO', 'PERCURSO', 'ATENDIMENTO TRATOU A SOLICITAÇÃO?');
...
function pesquisa_satisfacao2($uniqueid, $origem){
	...
	tracking_canal_ativo($canal, $ddr, $ticket, $indice);
        ...
        //18
        $indice++;
        tracking($canal, $ddr, $ticket, $indice, 'PESQUISA DE SATISFACAO', 'PERCURSO', 'FINALIZAÇÃO DA PESQUISA DE SATISFACAO');
        hangup();
...
//INTEGRAÇÃO VALIDA HORARIO
$horarioSevervico = true; //INTEGRAR CHAMADA REST  DA INTEGRAÇÃO, PENDENTE VALECARD
