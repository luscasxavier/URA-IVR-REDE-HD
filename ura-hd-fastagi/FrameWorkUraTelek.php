...
///////////////////////////////////////////////////////////////////////////////////////////////////////////
function verbose($texto,$loglevel='1')
{
global $configs, $debug_level, $origem, $fastagi, $uniqueid;
    if(!$debug_level){ $debug_level = 5;}
...
function canal_ativo($contador=0){

    global $configs,$fastagi;
    $status=$fastagi->channel_status();

    if($status["data"] == 'Line is up'){
        if($contador > 0){
            verbose("STATUS DO CANAL ATUALIZOU NO CONTADOR: ".$contador.", STATUS ATUAL DO CANAL :".$status["data"]);
        }
        return true;

    }elseif($status["data"] == 'Channel is down, but reserved' && $contador < 5){

        sleep(1);
        $contador = $contador+1;
        verbose("REPETINDO VALIDACAO DO CANAL POIS O MESMO NÃO SE ENCONTRA LINE IS UP,CONTADOR: ".$contador.", STATUS ATUAL DO CANAL :".$status["data"]);
        canal_ativo($contador);
...
/////////////////////////////////////////////////////////////////
function tracking_canal_ativo($canal, $ddr, $ticket, $indice, $contador = 0)
{
    global $configs,$fastagi;
    $status=$fastagi->channel_status();
    ...
    if($status["data"] == 'Line is up'){
        ...
        $contador = $contador+1;
        verbose("REPETINDO VALIDACAO DO CANAL POIS O MESMO NÃO SE ENCONTRA LINE IS UP,CONTADOR: ".$contador.", STATUS ATUAL DO CANAL :".$status["data"]);
        tracking_canal_ativo($canal, $ddr, $ticket, $indice, $contador);
...
function tracking($canal, $ddr, $ticket, $indice, $nome_evento, $extras_key, $extras_value){
    ...
    verbose("------------INICIO DO TRACKING ------------",1);
    verbose("TRACKING CANAL : ".$canal,1);
    verbose("TRACKING DDR : ".$ddr,1);
    verbose("TRACKING TICKET : ".$ticket,1);
    verbose("TRACKING INDICE : ".$indice,1);
    verbose("TRACKING NOME EVENTO : ".$nome_evento,1);
    verbose("TRACKING EXTRA KEY : ".$extras_key,1);
    verbose("TRACKING EXTRA VALUE : ".$extras_value,1);
    verbose("------------FIM DO TRACKING ------------",1);
